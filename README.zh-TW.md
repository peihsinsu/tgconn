...
| `tmp_retention_hours` | `TGCONN_TMP_RETENTION_HOURS` | —          | tmp/ 檔案保留時數，超過自動刪除（預設：24） |
| `log_retention_days`  | `TGCONN_LOG_RETENTION_DAYS`  | —          | 每日 `*.jsonl` 保留天數；`0` 表示停用（預設：30） |
| `history_max_entries` | `TGCONN_HISTORY_MAX_ENTRIES` | —          | 每個 `history_*.jsonl` 啟動 compact 後保留筆數；`0` 表示停用（預設：100） |
...
tmp_retention_hours: 24    # tmp/ 檔案保留時數
log_retention_days: 30     # 每日日誌保留天數；0 表示停用
history_max_entries: 100   # history compaction 上限；0 表示停用
```
...
任務持久化儲存在 `~/.tgconn/projects/<encoded-cwd>/cron/<id>.json`，重啟後自動重新載入。路徑編碼方式詳見 [儲存空間](#儲存空間)。
...
|------|------|
| 文字 | 直接轉發給 LLM |
| 圖片 | 下載至 `~/.tgconn/projects/<encoded-cwd>/tmp/<chat_id>/`，路徑與說明注入 prompt |
...
長按麥克風錄音 → 放開送出
    ↓
tgconn 下載 .ogg（Opus）到 ~/.tgconn/projects/<encoded-cwd>/tmp/<chat_id>/
    ↓
...
所有 per-project 狀態統一存放於 `~/.tgconn/projects/<encoded-cwd>/`，其中 `<encoded-cwd>` 是把絕對路徑中所有 `/` 替換為 `-`（沿用 Claude Code 的編碼方式）。例如 `/Users/alice/myproject` → `~/.tgconn/projects/-Users-alice-myproject/`。

```
~/.tgconn/projects/-Users-alice-myproject/
├── 2026-05-01.jsonl             ← 每日審計日誌（所有訊息）
├── history_123456789.jsonl      ← 每聊天室歷史（僅成功的對話）
├── tmp/<chat_id>/               ← 下載的圖片 / 檔案 / 語音
└── cron/
    ├── abc123.json              ← 排程任務定義
...
第一次啟動時，tgconn 會自動把舊版 `<cwd>/.tgconn/` 整個搬到中央位置，並在原處留下 `MOVED_TO_<encoded>.txt` 麵包屑檔，方便追蹤資料去向。

### Retention（啟動時執行一次）

| 設定鍵 | 預設值 | 行為 |
|--------|--------|------|
| `tmp_retention_hours` | `24` | 刪除 `tmp/` 內 mtime 超過 N 小時的檔案；空的 `<chat_id>/` 子目錄也會被移除 |
| `log_retention_days` | `30` | 刪除 N 天以上的每日 `*.jsonl`（專案根 + `cron/`）；`0` 表示停用 |
| `history_max_entries` | `100` | 把 `history_*.jsonl` 截斷至末尾 N 筆；`0` 表示停用 compaction |

`history_*.jsonl` 與 `cron/<job_id>.json` **不會**被 `log_retention_days` 觸及——history 由 `history_max_entries`（啟動時）或 `tgconn clean --history`（手動）管理；cron 任務定義僅能透過 `/cron del` 移除。

### 手動清理

```bash
tgconn clean --tmp                # 刪除所有 tmp 下載
tgconn clean --logs               # 刪除每日 *.jsonl（不含 history）
tgconn clean --history            # 刪除 history_*.jsonl（互動式 yes/no）
tgconn clean --all --dry-run      # 預覽全部要刪除的，不實際刪
tgconn clean --all --yes          # 全部清光，不詢問
```

`--history` 與 `--all` 需要輸入 `yes` 確認，除非加上 `--yes` 或 `--dry-run`。
...
└── internal/
    ...
    ├── cronjob/
    │   └── manager.go       Cron 排程器 + 任務持久化（路徑由 bot 注入）
    ├── downloader/
    │   └── downloader.go    Telegram 檔案下載（base dir 由 bot 注入）
    ├── provider/
    ...
    ├── storage/
    │   ├── resolver.go      解析 ~/.tgconn/projects/<encoded-cwd>/ 路徑
    │   ├── migrate.go       啟動時自動搬遷舊版 <cwd>/.tgconn/ 到中央位置
    │   └── cleanup.go       啟動時 retention（tmp、每日日誌、history compaction）
    └── transcriber/
